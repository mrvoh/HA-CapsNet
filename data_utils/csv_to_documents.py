...
def df_to_docs(
    ...
    use_excel=True,
    text_cols="text",
    target_prefix="topic_",
    binary_class=True,
):
    assert (
        0 < dev_percentage < 1
    ), "the percentage of data to be used for dev should be between 0 and 1."
    ...
    if do_split:
        ...
        train, _, dev, _ = iterative_train_test_split(
            df.values, y, test_size=dev_percentage
        )
    ...
    for df, name in dfs:
        docs = [
            Document(
                ...
                tags=[
                    idx_to_label[int(t)]
                    for t in np.argwhere(row[target_cols].values == 1)
                ],
...
def sheet_to_docs(
    ...
    assert (
        dev_percentage + test_percentage < 1
    ), "the percentage used for dev and test should be less than 1."
    ...
    label_to_idx = {
        col: ix
        for ix, col in enumerate(
            df[[col for col in df.columns if col.startswith(target_prefix)]].columns
        )
    }
    idx_to_label = {v: k for k, v in label_to_idx.items()}
    ...
    if isinstance(text_cols, list):
        df["text"] = df.apply(
            lambda x: " ".join([str(x[col]) for col in text_cols]), axis=1
        )
    else:
    ...
    if binary_class:
        ...
        target = (
            df[[col for col in df.columns if target_prefix in col]]
            .apply(np.nonzero, axis=1)
            .values
        )
        ...
        discounted_dev_percentage = dev_percentage / (1.0 - test_percentage)

        X_train, y_train, X_dev, y_dev = iterative_train_test_split(
            X, y, test_size=discounted_dev_percentage
        )
