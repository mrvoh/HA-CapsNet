...
def load_dataset(dataset_dir, dataset_name, num_threads=6, restructure_doc=True, split_size_long_seqs=50):
	"""
	...
	with concurrent.futures.ProcessPoolExecutor(num_threads) as executor:
		documents = list(tqdm.tqdm(executor.map(loader.read_file, filenames), total = len(filenames)))


	return documents

def parse(raw_data_dir, write_data_dir, dataset_name, num_tags, num_backtranslations=None, create_wordvecs=False, word_vec_dim=300, min_count=20, restructure_doc = True, split_size_long_seqs=50):
	out_paths = []
	...
	if num_tags: # sort and filter
		sorted_tags = sorted(label_occs.items(), key=operator.itemgetter(1), reverse=True)
	...
	label_to_idx_path = os.path.join(raw_data_dir,'label_to_idx_{}.json'.format(num_tags))
	...
	if num_backtranslations: #TODO: test backtranslation
		...
		doc_translations = parallel(delayed(doc.back_translate)(num_backtranslations) for doc in train_docs)
		...
		print("CREATED {} EXTRA TRAIN SAMPLES BY BACKTRANSLATION".format(len(doc_translations)))
		train_docs.extend(doc_translations)

	train_path = os.path.join(write_data_dir, '{}_train_{}.pkl'.format(dataset_name, num_tags))
