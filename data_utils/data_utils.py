...
def get_embedding(vecs, word_to_idx, embed_size):
    ...
    embed_table = [
        vecs[key] if key in word_to_idx.keys() else np.random.rand(embed_size)
        for key in sorted(word_to_idx.keys())
    ]
...
def _convert_word_to_idx(
    word, word_to_idx, word_counter=None, min_freq=None, unk="<UNK>"
):
...
def doc_to_sample(
    ...
    if stoi:  # preloaded word to idx, no need to update it
        sents = [
            torch.LongTensor([stoi.get(w, stoi[unk]) for w in sent])
        ...
        sents = [
            torch.LongTensor(
                [
                    _convert_word_to_idx(
                        w, word_to_idx, word_counter, min_freq_word, unk
                    )
                    for w in sent
                ]
            )
    ...
    if binary_class:
        ...
        sample["tags"] = torch.LongTensor(sample["tags"])
    else:
...
def get_data_loader(data, batch_size, drop_last, use_rnn, is_train=False):
    ...
    sampler = BucketBatchSampler(
        data, batch_size, drop_last=drop_last, sort_key=lambda row: -len(row["sents"])
    )
    # else:
    ...
    loader = DataLoader(
        data, batch_sampler=sampler, collate_fn=collate_fn, pin_memory=False
    )
    # shuffle=True,
    # num_workers=1)
    return loader
