...
class TextClassificationLearner:
    def __init__(
        ...
        K=5,
        verbose=True,
        gradual_unfeeze=True,
        keep_ulmfit_frozen=False,
        ...
        **kwargs
        ...
        if label_map:
            ...
            }  # Maps label ids to EUROVOC description
        ...
        self.writer = SummaryWriter(
            ...
            comment="Model={}-Labels={}-B={}-L2={}-Dropout={}".format(
                model_name, self.num_labels, B_train, weight_decay, dropout
            ),
        )
    ...
    def pred_to_labels(self, pred, top=None):
        ...
        if top:
            ...
            preds = list(np.nonzero(pred > 0.5))
    ...
    def predict_doc(self, doc):
        ...
        ), "Model needs to be initialized before inference can be done."
        self.model.eval()
        unk = "xxunk" if self.word_encoder.lower() == "ulmfit" else "<UNK>"
        sample, _ = doc_to_sample(
            doc, self.label_to_idx, None, stoi=self.word_to_idx, unk=unk
        )
        ...
        with torch.no_grad():
            (sents_batch, tags_batch, encoding_batch) = (
                collate_fn_rnn([sample])
                if self.word_encoder.lower() == "gru"
                else collate_fn_transformer([sample])
            )

            preds, word_attention_scores, sent_attention_scores, _ = self.model(
                sents_batch
            )
        ...
        sent_attention_scores = [
            l[0] for sublist in sent_attention_scores for l in sublist
        ]
        ...
        if len(sents_len) > 1:
            word_attention_scores = [
                score[:l] for l, score in zip(sents_len, word_attention_scores)
            ]
        else:
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            if (
                (self.word_encoder.lower() == "ulmfit")
                and (epoch <= to_freeze)
    ...
    def _eval_model(
        self, dataloader_train, dataloader_dev, best_score, best_loss, train_step
    ):
        ...
        (
            ...
            f1_micro_dev,
            f1_macro_dev,
        ) = self.eval_dataset(dataloader_dev, K=self.K, write_path=write_path)
        ...
        (
            ...
            dataloader_train, K=self.K, max_samples=len(dataloader_dev)
        )
        ...
        if best_score <= f1_micro_dev:
            ...
            self.logger.info(
                "Saved model with new best score: {0:.3f}".format(best_score)
            )
        ...
        self.writer.add_scalars(
            "Loss", {"Train": avg_loss_tr, "Dev": avg_loss_dev}, train_step
        )
        self.writer.add_scalars(
            "R_{}".format(self.K), {"Train": r_k_tr, "Dev": r_k_dev}, train_step
        )
        self.writer.add_scalars(
            "P_{}".format(self.K), {"Train": p_k_tr, "Dev": p_k_dev}, train_step
        )
        self.writer.add_scalars(
            "RP_{}".format(self.K), {"Train": rp_k_tr, "Dev": rp_k_dev}, train_step
        )
        self.writer.add_scalars(
            "NDCG_{}".format(self.K),
            {"Train": ndcg_k_tr, "Dev": ndcg_k_dev},
            train_step,
        )
    ...
    def eval_dataset(self, dataloader, K=0, max_samples=None, write_path=None):
        ...
        hamming, emr, f1_micro, f1_macro, roc_auc_micro = accuracy(
            y_true, y_pred, False, self.binary_class
        )

        if write_path is not None:
            write_classification_report(
                write_path, y_pred, y_true, self.label_to_idx, False, self.binary_class
            )  # TODO: remove hard-coded stuff

        log_results = "Hamming score {:1.3f} | Exact Match Ratio {:1.3f} | Micro F1 {:1.3f} | Macro F1 {:1.3f}".format(
            hamming, emr, f1_micro, f1_macro
        )
