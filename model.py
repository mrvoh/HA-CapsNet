    ...
    def train(
        ...
        if not optimize_time:
            self.model = fasttext.train_supervised(
                train_path,
                loss="ova" if binary_classification else "hs",
                lr=lr,
                epoch=epoch,
                dim=embed_size,
                minn=minn,
            ...
            assert (
                dev_path
            ), "When FastText is optimized, a development set must also be given"
            self.model = fasttext.train_supervised(
                train_path,
                autotune_validation_file=dev_path,
                autotune_duration=optimize_time,
                loss="hs" if binary_classification else "ova",
            )
        ...
        if test_path:
            ...
            print(
                "FastText model attained P@{0}: {1:0.3f}, R@{0}: {2:0.3f}, F1@{0}: {3:0.3f}".format(
                    K, p_k, r_k, (2 * p_k * r_k) / (r_k + p_k)
                )
            )
    ...
    def __init__(
        ...
        **kwargs
        ...
        if word_encoder.lower() == "ulmfit":
            ...
            word_out = (
                2 * word_hidden
                if (bidirectional and word_encoder.lower() == "gru")
                else word_hidden
            )
        sent_out = (
            2 * sent_hidden
            if (bidirectional and sent_encoder.lower() == "gru")
            else sent_hidden
        )
        ...
        self.doc_encoder = AttentionSentEncoder(
            sent_encoder,
            sent_hidden,
            word_out,
            bidirectional=bidirectional,
            num_layers=num_layers_sen,
            nhead=nhead_sen,
        )
...
class HGRULWAN(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.doc_encoder = GRUMultiHeadAtt(
            sent_hidden,
            word_out,
            nhead_doc=num_classes,
            bidirectional=bidirectional,
            dropout=dropout,
...
class HCapsNet(nn.Module):
    """"
    Hierarchical Attention Network (HAN) with CapsNet classification head
    """
    def __init__(
        self,
        num_tokens,
        ...
        **kwargs
        ...
        self.caps_classifier = CapsNet_Text(
            1,
            sent_out,
            num_classes,
            dim_caps=dim_caps,
...
class HierarchicalAttentionCapsNet(nn.Module):
    """"
    Hierarchical (Multihead) Self Attention Capsule Network
    """
    def __init__(
        self,
        num_tokens,
        ...
        **kwargs
        ...
        super(HierarchicalAttentionCapsNet, self).__init__()
